...
2、使用如下用法时，程序会自动判断是否是insert还是update
    local obj＝ dbtech.new(data);
3、使用如下用法时，程序会自动判断是否是insert还是update
    local obj＝ dbtech.new();
    obj:init(data);
]]
...
function dbtech:ctor(v)
    self.__name__ = "tech"    -- 表名
...
function dbtech:init(data, isNew)
    data = dbtech.validData(data)
    self.__key__ = data.idx
    local hadCacheData = false
    if self.__isNew__ == nil and isNew == nil then
        local d = skynet.call("CLDB", "lua", "get", dbtech.name, self.__key__)
        if d == nil then
            d = skynet.call("CLMySQL", "lua", "exesql", dbtech.querySql(data.idx))
...
function dbtech:getInsertSql()
    if self:isEmpty() then
        return nil
    end
    local data = dbtech.validData(self:value2copy())
    local sql = skynet.call("CLDB", "lua", "GETINSERTSQL", self.__name__, data)
    return sql
end
function dbtech:tablename() -- 取得表名
    return self.__name__
end

function dbtech:value2copy()  -- 取得数据复样，注意是只读的数据且只有当前时刻是最新的，如果要取得最新数据及修改数据，请用get、set
...
function dbtech:refreshData(data)
    if data == nil or self.__key__ == nil then
        skynet.error("dbtech:refreshData error!")
        return
    end
...
function dbtech:isEmpty()
    return (self.__key__ == nil) or (self:get_idx() == nil)
end

function dbtech:release(returnVal)
    local val = nil
...
function dbtech:setTrigger(server, cmd, fieldKey)
...
function dbtech.querySql(idx)
    ...
    if #where > 0 then
        return "SELECT * FROM tech WHERE " .. table.concat(where, " and ") .. ";"
    else
       return "SELECT * FROM tech;"
    end
end
...
function dbtech.getListBycidx(cidx, forceSelect, orderby, limitOffset, limitNum)
    ...
    local groupInfor = skynet.call("CLDB", "lua", "GETGROUP", dbtech.name,  groupKey) or {}
    cachlist = groupInfor[1] or {}
    isFullCached = groupInfor[2]
    if isFullCached == true and (not forceSelect) then
        list = cachlist
        for k, v in pairs(list) do
            data = dbtech.new(v, false)
            table.insert(ret, data:value2copy())
            data:release()
        end
    else
        local sql = "SELECT * FROM tech WHERE cidx=" .. cidx ..  (orderby and " ORDER BY" ..  orderby or "") .. ((limitOffset and limitNum) and (" LIMIT " ..  limitOffset .. "," .. limitNum) or "") .. ";"
        list = skynet.call("CLMySQL", "lua", "exesql", sql)
        if list and list.errno then
            skynet.error("[dbtech.getGroup] sql error==" .. sql)
            return nil
         end
     ...
     list = nil
     -- 设置当前缓存数据是全的数据
     skynet.call("CLDB", "lua", "SETGROUPISFULL", dbtech.name, groupKey)
     return ret
end

function dbtech.validData(data)
...
function dbtech.instanse(idx)
    if type(idx) == "table" then
        local d = idx
        idx = d.idx
    end
    if idx == nil then
        skynet.error("[dbtech.instanse] all input params == nil")
        return nil
    end
    local key = (idx or "")
    if key == "" then
    ...
    ---@type dbtech
    local obj = dbtech.new()
    local d = skynet.call("CLDB", "lua", "get", dbtech.name, key)
    if d == nil then
        d = skynet.call("CLMySQL", "lua", "exesql", dbtech.querySql(idx))
        if d and d.errno == nil and #d > 0 then
            if #d == 1 then
                d = d[1]
                -- 取得mysql表里的数据
                ...
                error("get data is more than one! count==" .. #d .. ", lua==dbtech")
            end
        else
            -- 没有数据
            obj.__isNew__ = true
        ...
        obj.__isNew__ = false
        obj.__key__ = key
        skynet.call("CLDB", "lua", "SETUSE", dbtech.name, key)
    end
    return obj
end
